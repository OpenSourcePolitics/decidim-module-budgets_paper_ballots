...
describe "Admin manages projects" do
  ...
  describe "bulk actions" do
    ...
    describe "update projects budget" do
      ...
      let!(:paper_ballot_result2) { create :paper_ballot_result, project: project2, votes: 15 }
      ...
      it "shows the paper ballots count" do
        visit current_path
        within find_all(".card-divider").last do
          expect(page).to have_content("Paper ballots: 35")
        end

        within "tr[data-id='#{project.id}']" do
          within "td[data-label='Votes count']" do
            expect(page).to have_content("20 (0/20)")
          end
        end

        within "tr[data-id='#{project2.id}']" do
          within "td[data-label='Votes count']" do
            expect(page).to have_content("15 (0/15)")
          end
        end
      end
